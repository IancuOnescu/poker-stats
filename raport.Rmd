...
plot_top_hands_and_store = function(i, hands) {
  plot_top_hands(pdata, as.character(pfreq$p_name[i]), hands)
  pthlist = recordPlot()
  plot.new()
  return (pthlist)
}
...
for (i in 1:50) {
  ...
  pthlist5 = plot_top_hands_and_store(i, 5)
  pthlist10 = plot_top_hands_and_store(i, 10)
  pthlist15 = plot_top_hands_and_store(i, 15)
  rordf = table_RoR(pdata, as.character(pfreq$p_name[i]))
  ...
  f = sprintf("top_hands_5_%s.rds", fpfreq$p_name[i])
  saveRDS(pthlist5, f)
  thlist5_file = append(pthlist5_file, f)
  
  f = sprintf("top_hands_10_%s.rds", fpfreq$p_name[i])
  saveRDS(pthlist10, f)
  thlist10_file = append(pthlist10_file, f)
  
  f = sprintf("top_hands_15_%s.rds", fpfreq$p_name[i])
  saveRDS(pthlist15, f)
  thlist15_file = append(pthlist15_file, f)
  
  f = sprintf("risk_of_ruin_%s.rds", fpfreq$p_name[i])
  saveRDS(rordf, f)
  tror_file = append(ror_file, f)
...
dfp = data.frame(unlist(p_name), 
                 ...
                 unlist(dist1e4_file),
                 unlist(thlist5_file),
                 unlist(thlist10_file),
                 unlist(thlist15_file),
                 unlist(tror_file))
colnames(dfp) = c("p_name",
                  ...
                  "thlist15_file",
                  "tror_file")
write_feather(dfp, "outcomes_and_bankrolls_and_tophands.feather")
```
