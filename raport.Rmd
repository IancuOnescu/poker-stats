...
for(i in 2:5){
  ...
  plot_player_distr(pdata, name, 10, prefiltered = TRUE)
...
for(i in 5:10){
  pdata = filter(data, p_name == fpfreq$p_name[i])
  df = table_RoR(pdata, as.character(fpfreq$p_name))
  grid.table(df)
}
gc()
```
...
for(i in 5:10){
  pdata = filter(data, p_name == fpfreq$p_name[i])
  plot_top_hands(pdata, as.character(fpfreq$p_name), 5)
}
gc()
```
...
## Referinte
*in afara de carte, articol si site-ul de unde am luat datasetul ce referinte mai avem?